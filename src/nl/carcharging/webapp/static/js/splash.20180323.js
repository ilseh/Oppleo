...
//  js object
//  power
var hsDevices = [[ 298, 'Woonkamer',	'SplashLightDevice',	null ], // Woonkamer Buislamp
...
function timestamp() {
	const now = new Date();
	return now.getFullYear() + "-" + (now.getMonth()<10?'0':'') + (now.getMonth()+1) + "-" + (now.getDate()<10?'0':'') + now.getDate() + '.' + (now.getHours()<10?'0':'') + now.getHours() + ":" + (now.getMinutes()<10?'0':'') + now.getMinutes() + "." + (now.getSeconds()<10?'0':'') + now.getSeconds() + '.' + (now.getMilliseconds()<100?'0':'') + (now.getMilliseconds()<10?'0':'') + now.getMilliseconds();
}
...
function Settings() {
	...
	if ( !$('div#settings-modal').length ) {
                        ...
                        <div class="row">
                            ...
                            <table class="table table-striped">
                               ...
                               <tr id="CentralHSUpdater" class="settings_row">
                                    ...
                                    <td class="settings_col_7"><i class="fa fa-circle-o-notch fa-lg fa-spin"> </i></th>
                                </tr>                               
                               <tr id="CentralEHUpdater" class="settings_row">
                                    ...
                                    <td class="settings_col_5 text-center">...</td>
                                    <td class="settings_col_6 text-center">0</td>
                                    <td class="settings_col_7"><i class="fa fa-pause-circle-o fa-lg"> </i></th>
                                </tr>
		...
		$('tr#CentralBoseUpdater td:nth-of-type(7)').on('click', function() {			
			console.log( timestamp() + ' CentralBoseUpdater setting clicked...');
			if ( CentralBoseUpdater.isRunning() ) {
				CentralBoseUpdater.terminate();
			} else {
				CentralBoseUpdater.restart();
			};
			var isRunning = CentralBoseUpdater.isRunning();
			$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-play-circle-o', !isRunning );
			$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-circle-o-notch', isRunning );
			$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-spin', isRunning );
		});
	}
	// Update
	
	$('tr#CentralBoseUpdater td:nth-of-type(3)').html( ( CentralBoseUpdater.getInterval() / 1000) + "s" );
	$('tr#CentralBoseUpdater td:nth-of-type(4)').html( CentralBoseUpdater.elementCount() );
	$('tr#CentralBoseUpdater td:nth-of-type(5)').html( CentralBoseUpdater.callbackCount() );
	var isRunning = CentralBoseUpdater.isRunning();
	$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-play-circle-o', !isRunning );
	$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-circle-o-notch', isRunning );
	$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-spin', isRunning );
	
	$('tr#CentralNRUpdater td:nth-of-type(3)').html( ( CentralNRUpdater.getInterval() / 1000) + "s" );
	$('tr#CentralNRUpdater td:nth-of-type(4)').html( CentralNRUpdater.elementCount() );
	$('tr#CentralNRUpdater td:nth-of-type(5)').html( CentralNRUpdater.callbackCount() );
	
	$('tr#CentralEHUpdater td:nth-of-type(3)').html( ( CentralEHUpdater.getInterval() / 1000) + "s" );
	$('tr#CentralEHUpdater td:nth-of-type(4)').html( CentralEHUpdater.elementCount() );
	$('tr#CentralEHUpdater td:nth-of-type(5)').html( CentralEHUpdater.callbackCount() );

	$('tr#CentralHSUpdater td:nth-of-type(3)').html( ( CentralHSUpdater.getInterval() / 1000) + "s" );
	$('tr#CentralHSUpdater td:nth-of-type(4)').html( CentralHSUpdater.elementCount() );
	$('tr#CentralHSUpdater td:nth-of-type(5)').html( CentralHSUpdater.callbackCount() );

	$('tr#CentralAudiVehicleUpdater td:nth-of-type(3)').html( ( CentralAudiVehicleUpdater.getInterval() / 1000) + "s" );
	$('tr#CentralAudiVehicleUpdater td:nth-of-type(4)').html( CentralAudiVehicleUpdater.elementCount() );
	$('tr#CentralAudiVehicleUpdater td:nth-of-type(5)').html( CentralAudiVehicleUpdater.callbackCount() );

	$('tr#CentralLife360Updater td:nth-of-type(3)').html( ( CentralLife360Updater.getInterval() / 1000) + "s" );
	$('tr#CentralLife360Updater td:nth-of-type(4)').html( CentralLife360Updater.elementCount() );
	$('tr#CentralLife360Updater td:nth-of-type(5)').html( CentralLife360Updater.callbackCount() );

	$("#settings-modal").modal();
}
function flashSettings( id, cnt ) {
	// Only flash if visible
	$('tr#' + id +' td:nth-of-type(6)').html( cnt );	
	if ( $('div#settings-modal').is(':visible') && $('tr#' + id).css('opacity') == 1 ) {	// Don't start the fading if it is already animating
		$('tr#' + id).fadeOut(50).fadeIn(100);
		var isRunning = CentralBoseUpdater.isRunning();
		$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-play-circle-o', !isRunning );
		$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-circle-o-notch', isRunning );
		$('tr#CentralBoseUpdater td:nth-of-type(7) i').toggleClass( 'fa-spin', isRunning );
...
// ----------------------- Bose SoundTouch Object ------------------------------------------
// Central updater for Bose SoundTouch 

var CentralBoseUpdater = {
    ips: {},
    handles: {},
    startTimes: {},
	callActive: {},
	...
	isRunning: function() {
		...
		var localHandlesRef = this.handles;
		var localCallActiveRef = this.callActive;
    	var isRunning = false, ar = Object.keys( this.handles );
    	$.each( ar, function( ei, ev ) {
    		if ( ( typeof localHandlesRef[ ev ] !== 'undefined' && localHandlesRef[ ev ] != null ) || 
    			 ( typeof localCallActiveRef[ ev ] !== 'undefined' && localCallActiveRef[ ev ] != null ) ) {
    			isRunning = true;
    ...
    elementCount: function() {
    	return Object.keys( this.ips ).length;
    },
    callbackCount: function() {
    	var i = 0, ar = this.ips;
    	$.each( Object.keys(ar), function( ei, ev ) {
			i += ar[ev].length;
		});
    	return i;
    },
    ...
    setInterval: function( interval ) {
    	this.interval = Math.max( interval, this.minInterval );
    },
    register: function( ip, callback ) {
		console.log( timestamp() + " CentralBoseUpdater.register() - registering callback for " + ip );
    	if ( !( ip in this.ips ) ) {
			// First entry, create array
		   	this.ips[ ip ] = new Array();
			// Add handler
		   	this.ips[ ip ].push( callback );
			// // This is a new location, start the updater for it
			CentralBoseUpdater.update( ip );
    	} else {
	...
	terminate: function( ) {
		...
		var localHandlesRef = this.handles;
		var localStartTimesRef = this.startTimes;
		var localCallActiveRef = this.callActive;
		$.each( keys, function( index, key ) {
			clearTimeout( localHandlesRef[ key ] );
			localHandlesRef[ key ] = null;
			localStartTimesRef[ key ] = null;
			if ( typeof localCallActiveRef[ key ] !== 'undefined' && localCallActiveRef[ key ] != null ) {
				localCallActiveRef[ key ].abort();
				localCallActiveRef[ key ] = null;
			}
		});
	},
	restart: function( ) {
		var ips = Object.keys( this.ips );
		$.each( ips, function( index, ip ) {
			// // Restart the updater for it
			CentralBoseUpdater.update( ip );
		});
	},
	notify: function ( soundTouch ) {
		console.log( timestamp() + " CentralBoseUpdater.notifier() - calling callbacks for " + soundTouch.getIP() );
		$.each( this.ips[ soundTouch.getIP() ], function( index, callback ) {
		...
		flashSettings('CentralBoseUpdater', ++this.notifyCnt );
    },
    update: function ( ip ) {				// can be called pn intervals, or just tp initiate a new update now
    	// If an update is active, ignore this
    	if ( typeof this.callActive[ ip ] !== 'undefined' && this.callActive[ ip ] != null ) {
    		console.log( timestamp() + " CentralBoseUpdater.update() called for " + ip + " ignored, request in progress...");
    		return;
		}
    	...
    	if ( typeof this.handles[ ip ] !== 'undefined' && this.handles[ ip ] != null ) {
			clearTimeout( this.handles[ ip ] );
		}    	
		this.handles[ ip ] = null;
		this.startTimes[ ip ] = null;
		var localHandlesRef = this.handles;
		var localStartTimesRef = this.startTimes;
		var localCallActiveRef = this.callActive;
		var jqxhr = $.getJSON( "assets/php/bose.getstatus.php?ip=" + ip, function() {
			
			console.log( timestamp() + " CentralBoseUpdater.update() - Successful request for " + ip );
			localCallActiveRef[ ip ] = this;
		})
		.done(function( data ) {
			console.log( timestamp() + " CentralBoseUpdater.update() - Successful response for " + data.ip );
			...
			CentralBoseUpdater.notify( soundTouch );
		})
		.fail(function( jqxhr, textStatus, error ) {
			console.log( timestamp() + " CentralBoseUpdater.update() - Error retrieving soundTouch with ip " + ip + ". " + textStatus + ", " + error );
			$.Notification.notify('error','top left', 'Connection error', 'Could not retrieve data from Bose SoundTouch!' );
		})
		.always(function() {
			console.log( timestamp() + " CentralBoseUpdater.update() - Done " + ip + ", again in " + CentralBoseUpdater.getInterval() );
			// Update again in x seconds
			if ( this == localCallActiveRef[ ip ] ) {
				localHandlesRef[ ip ] = setTimeout( function() { CentralBoseUpdater.update( ip ) }, CentralBoseUpdater.getInterval() );
				localStartTimesRef[ ip ] = new Date();
				localCallActiveRef[ ip ] = null;
			}
		});
    }
}
...
var CentralNRUpdater = {
	...
	notifyCnt: 0,
    elementCount: function() {
    	return Object.keys(this.serials).length;
    },
    callbackCount: function() {
    	var i = 0, ar = this.serials;
    	$.each( Object.keys(ar), function( ei, ev ) {
			i += ar[ev].length;
		});
    	return i;
	...
	notify: function ( robot ) {
		...
		flashSettings('CentralNRUpdater', ++this.notifyCnt );
    },
...
var CentralEHUpdater = {
	...
	notifyCnt: 0,
    elementCount: function() {
    	return Object.keys(this.locationIds).length;
    },
    callbackCount: function() {
    	var i = 0, ar = this.locationIds;
    	$.each( Object.keys(ar), function( ei, ev ) {
			i += ar[ev].length;
		});
    	return i;
	...
	notify: function ( locationObj ) {
		...
		flashSettings('CentralEHUpdater', ++this.notifyCnt );
    },
...
var CentralHSUpdater = {
	...
	notify: function ( hsDev ) {
		...
		flashSettings('CentralHSUpdater', ++this.notifyCnt );
    },
...
SplashSoundTouch.prototype.draw = function( ) {
	...
	if ( $('div#bstd_' + ipId).length ) {
		...
		var obj = this;
		CentralBoseUpdater.register( ip, function() { obj.draw() } );
...
//		$( this.getBindingElement() ).append('<div class="' + this.getWidthClass() + '" id="slrd"><div class="card-box"><form class="form-horizontal"><div class="form-group"><label for="slrd_slider" class="control-label"><b>Woonkamer</b> <span class="font-normal text-muted clearfix">dimmer</span></label><div class="m-b-20"><input type="text" id="slrd_slider"></div><div><div class="slrdb_activity_wrapper"><div class="btn-group m-b-10"><button type="button" id="slrdb_on" class="slrdb_on btn btn-light waves-effect"> <i class="fa fa-toggle-on m-r-5"></i> <span>Aan</span></button><button type="button" id="slrdb_off" class="slrdb_off btn btn-outline-light waves-effect waves-dark"> <i class="fa fa-toggle-off m-r-5"></i> <span>Uit</span></button></div>&nbsp;<div class="btn-group m-b-10"><button type="button" id="slrdb_plus" class="slrdb_plus btn btn-light waves-effect waves-light"> <i class="fa fa-chevron-up m-r-5"></i> </button><button type="button" id="slrdb_min" class="slrdb_min btn btn-outline-light waves-effect waves-light"> <i class="fa fa-chevron-down m-r-5"> </i> </button></div>&nbsp;<div class="btn-group m-b-10"><button type="button" id="slrdb_movie" class="slrdb_movie btn btn-outline-info waves-effect waves-light"><i class="fa fa-television m-r-5"></i> <span>Film</span></button><button type="button" id="slrdb_entertain" class="slrdb_entertain btn btn-outline-success waves-effect waves-light"><i class="fa fa-comments m-r-5"></i> <span>Visite</span></button><button type="button" id="slrdb_kook" class="slrdb_kook btn btn-outline-warning waves-effect waves-light"><i class="fa fa-search m-r-5"></i> <span>Koken</span></button><button type="button" id="slrdb_bright" class="slrdb_bright btn btn-outline-danger waves-effect waves-light"><i class="fa fa-sun-o m-r-5"></i> <span>Bright</span></button></div></div></div> </div></form></div></div>');
		...
		// Register for updates. Bind the callback to this object
		var obj = this;
		CentralHSUpdater.register( 298, function() { obj.draw() } );	// Woonkamer Buislamp
		CentralHSUpdater.register( 377, function() { obj.draw() } );	// Woonkamer Kleurenlamp
		CentralHSUpdater.register( 422, function() { obj.draw() } );	// Woonkamer tripodlamp
		CentralHSUpdater.register( 441, function() { obj.draw() } );	// Keuken plafondspots
		CentralHSUpdater.register( 508, function() { obj.draw() } );	// Gang Designlamp
		CentralHSUpdater.register( 738, function() { obj.draw() } );	// Woonkamer Vlechtlamp
		CentralHSUpdater.register( 814, function() { obj.draw() } );	// Woonkamer Eettafel
	}
...
function setValueForHSDevice( ref, value, notify = true ) {
	...
	hsd.setValue( value );
	if ( notify ) { CentralHSUpdater.notify( hsd ); }
...
function SplashSoundTouchUpdate( ip, action, volume = null, url = null, notification = null ) {
	...
	.done(function( data ) {
		...
		// Retrieve the updated status
		CentralBoseUpdater.update( data.ip );
	})
	.fail(function( jqxhr, textStatus, error ) {
...
// ----------------------- Audi Vehicles Object ------------------------------------------
// Central updater for Audi Vehicles
// TODO: all VINs (vehicles) are reported in a single status

var CentralAudiVehicleUpdater = {
    vins: new Array(),
    interval: 60000,	// Use 1 minute
    isTerminated: false,
    notifyCnt: 0,
    elementCount: function() {
    	return Object.keys(this.vins).length;
    },
    callbackCount: function() {
    	var i = 0, ar = this.vins;
    	$.each( Object.keys(ar), function( ei, ev ) {
			i += ar[ev].length;
		});
    	return i;
    ...
    register: function( vin, callback ) {
		console.log( timestamp() + " CentralAudiVehicleUpdater.register() - registering callback for " + vin );
    	if ( !( vin in this.vins ) ) {
			// First entry, create array
		   	this.vins[ vin ] = new Array();
			// Add handler
		   	this.vins[ vin ].push( callback );
			// // This is a new location, start the updater for it
			CentralAudiVehicleUpdater.update( vin );
    	} else {
			// Else just add handler
		   	this.vins[ vin ].push( callback );
    	}
	...
	notify: function ( member ) {
		console.log( timestamp() + " CentralAudiVehicleUpdater.notifier() - calling callbacks for " + member.getVin() );
		$.each( this.vins[ member.getVin() ], function( index, callback ) {
			// Make sure the callback is a function
			if (typeof callback === "function") {
				// Call it, since we have confirmed it is callable
				callback( member );
		...
		flashSettings('CentralAudiVehicleUpdater', ++this.notifyCnt );
    },
    update: function ( vin ) {
    	if ( this.isTerminated ) {
    		return;
    	}
		var jqxhr = $.getJSON( "assets/php/audi.getstatus.php", function() {
			console.log( timestamp() + " CentralAudiVehicleUpdater.update() - Successful request for " + vin );
		})
		.done(function( data ) {
			console.log( timestamp() + " CentralAudiVehicleUpdater.update() - Successful response" );
			...
			$.each( data.userVehicles, function( index, vehicleData ) {
				...
				// Now notify the callbacks
				CentralAudiVehicleUpdater.notify( vehicle );
			});
		})
		.fail(function( jqxhr, textStatus, error ) {
			console.log( timestamp() + " CentralAudiVehicleUpdater.update() - Error retrieving member with vin " + vin + ". " + textStatus + ", " + error );
			$.Notification.notify('error','top left', 'Connection error', 'Could not retrieve information from Audi!' );
		})
		.always(function() {
			console.log( timestamp() + " CentralAudiVehicleUpdater.update() - Done " + vin + ", again in " + CentralAudiVehicleUpdater.getInterval() );
			// Update again in x seconds
			setTimeout( function() { CentralAudiVehicleUpdater.update( vin ) }, CentralAudiVehicleUpdater.getInterval() );
		});
    }
}
...
SplashAudiVehicleMember.prototype.draw = function( ) {
	...
	if ( $('div#audi_' + vin).length ) {
		...
		var obj = this;
		CentralAudiVehicleUpdater.register( vin, function() { obj.draw() } );
...
// ----------------------- Life360 Object ------------------------------------------
// Central updater for Life360
// TODO: all members and circles are reported in a single status

var CentralLife360Updater = {
    ids: {},
    interval: 60000,	// Use 1 minute
    isTerminated: false,
	notifyCnt: 0,
    ...
    register: function( id, callback ) {
		console.log( timestamp() + " CentralLife360Updater.register() - registering callback for " + id );
    	if ( !( id in this.ids ) ) {
			// First entry, create array
		   	this.ids[ id ] = new Array();
			// Add handler
		   	this.ids[ id ].push( callback );
			// // This is a new location, start the updater for it
			CentralLife360Updater.update( id );
    	} else {
			// Else just add handler
		   	this.ids[ id ].push( callback );
    	}
	...
	notify: function ( member ) {
		console.log( timestamp() + " CentralLife360Updater.notifier() - calling callbacks for " + member.getName() );
		$.each( this.ids[ member.getId() ], function( index, callback ) {
			// Make sure the callback is a function
			if (typeof callback === "function") {
				// Call it, since we have confirmed it is callable
				callback( member );
		...
		flashSettings('CentralLife360Updater', ++this.notifyCnt );
    },
    update: function ( id ) {
    	if ( this.isTerminated ) {
    		return;
    	}
		var jqxhr = $.getJSON( "assets/php/life360.getstatus.php", function() {
			console.log( timestamp() + " CentralLife360Updater.update() - Successful request for " + id );
		})
		.done(function( data ) {
			console.log( timestamp() + " CentralLife360Updater.update() - Successful response" );
			...
			$.each( data.circles, function( index, circleData ) {
				$.each( circleData.member, function( index, memberData ) {
					...
					// TODO: now when a member exists in more than 1 circle, it is notified for each occurance

					// Now notify the callbacks
					CentralLife360Updater.notify( member );
				});
			});			
		})
		.fail(function( jqxhr, textStatus, error ) {
			console.log( timestamp() + " CentralLife360Updater.update() - Error retrieving member with id " + id + ". " + textStatus + ", " + error );
			$.Notification.notify('error','top left', 'Connection error', 'Could not retrieve location data from Life360!' );
		})
		.always(function() {
			console.log( timestamp() + " CentralLife360Updater.update() - Done " + id + ", again in " + CentralLife360Updater.getInterval() );
			// Update again in x seconds
			setTimeout( function() { CentralLife360Updater.update( id ) }, CentralLife360Updater.getInterval() );
		});
    }
}
...
SplashLife360Member.prototype.draw = function( ) {
	...
	if ( $('div#life360_' + id).length ) {
		...
		var obj = this;
		CentralLife360Updater.register( id, function() { obj.draw() } );
