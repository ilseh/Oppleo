...
class WebSocketThread(object):
    ...
    def __init__(self):
        self.logger = logging.getLogger('nl.oppleo.models.SessionModel')
    ...
    def websocket_start(self):
        ...
        while not self.stop_event.is_set():
            ...
            if (time.time() *1000.0) > (lastRun + (OppleoConfig.device_measurement_check_interval *1000.0)):
                # time to run again
                try:
    ...
    def websocket_send_usage_update(self, type):
        ...
        qr = device_measurement.get_last_saved(energy_device_id=OppleoConfig.ENERGY_DEVICE_ID)
